...
- **Team**: **Engineering** — key `ENGG`, id `733dcec1-824d-44a6-b023-0f46777b6a8a`. Workspace: `linear.app/docudex`. Every `save_issue` that creates an issue needs this `teamId`; issue identifiers look like `ENGG-123`.
...
- **Read an issue**: `get_issue` by id/identifier (e.g. `ENGG-123`). Use `list_comments` for its discussion.
...
- **Blocking**: use Linear's native issue relations (`blocks` / `blocked by`). Where a relation can't be set, fall back to a `Blocked by: ENGG-<n>, ENGG-<n>` line at the top of the child body. A ticket is unblocked when every blocker is in a completed/canceled state.
